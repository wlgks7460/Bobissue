    ...
    <div className='w-[1300px]'>
      ...
      <div className='mb-6 ml-[24px]'>
        {categories.map((category) => (
          <button
            key={category}
            className={`px-6 py-2 w-[150px] text-sm font-medium transition ${
      ...
      <div className=' bg-white rounded-md p-6 w-[1250px]'>
            ...
            {filteredNotices.map((notice) => (
              <tr key={notice.id} className='border-b hover:bg-gray-50'>
                ...
                <td className={`px-4 py-2 text-center font-semibold ${
                  notice.category === '필독' ? 'text-red-500'
                  : notice.category === '경고' ? 'text-yellow-500'
                  : notice.category === '안전' ? 'text-green-500'
                  : 'text-gray-700'
                }`}>
                  {notice.category}
                </td>
                <td onClick={() => HandleClicknavigate(notice.id)} className='px-4 py-2 cursor-pointer'>
                  {notice.title}
                </td>
